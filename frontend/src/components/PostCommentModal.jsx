...
function PostCommentModal({ open, onClose, postId, onNewComment }) {
  ...
  const fetchComments = async (pageParam) => {
    ...
    try {
      const res = await fetchWith401Check(`${API_BASE_URL}/comments/${postId}/comments?page=${pageParam}`, {
        method: "GET",
        credentials: "include",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json();
      if (!data.content || data.content.length === 0) { setHasMore(false); return; }
      setComments(prev => [...prev, ...data.content.map(c => ({ ...c }))]); // clone để Chrome render
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => { if (page > 0) fetchComments(page); }, [page]);
  ...
  const handleSendComment = async () => {
    ...
    try {
      ...
      const newCommentWithPermission = { ...data, canEdit: true, canDelete: true };
      setComments(prev => [newCommentWithPermission, ...prev.map(c => ({ ...c }))]); // clone prev
      setNewComment("");
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      await fetchWith401Check(`${API_BASE_URL}/comments/${commentId}`, { method: "DELETE", credentials: "include", headers: { Authorization: `Bearer ${getToken()}` } });
      setComments(prev => prev.filter(c => c.id !== commentId).map(c => ({ ...c }))); // clone
  ...
  const handleEditComment = async (comment) => {
    ...
    try {
      const res = await fetchWith401Check(`${API_BASE_URL}/comments/${comment.id}`, {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` },
        body: JSON.stringify({ content: updatedContent, postId, parentCommentId: comment.parentCommentId || null }),
      });
    ...
    >
      {loadingPost ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
          <Card style={{ margin: 12, background: "#2f2f2f", color: "white", maxHeight: "40%", overflowY: "auto" }}>
          ...
          <div ref={commentContainerRef} onScroll={handleScroll} style={{ flex: 1, overflowY: "auto", padding: "0 12px" }}>
            {comments.map(comment => {
              ...
              return (
                <Card key={`${comment.id}-${comment.content}`} size="small" style={{ marginBottom: 8, background: "#3a3a3a", color: "white" }}
                  extra={menuItems.length > 0 ? (
                  ...
                  <span style={{ fontSize: 12, color: "#dcdcdc" }}>{dayjs(comment.createdAt).format("HH:mm DD/MM/YYYY")}</span>
                  <p style={{ marginTop: 4 }}>{comment.content}</p>
                  <p style={{ fontSize: 12, color: "#dcdcdc" }}>❤️ {comment.reactionCount}</p>
                </Card>
              );
            })}
            {loadingComments && <div style={{ textAlign: "center", marginTop: 8 }}><Spin size="small" /></div>}
            {!hasMore && <p style={{ textAlign: "center", color: "#dcdcdc" }}>Hết bình luận</p>}
          </div>

          <div style={{ display: "flex", gap: 8, padding: "8px 12px", borderTop: "1px solid #444", background: "#242424" }}>
            <Input value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Viết bình luận..." style={{ flex: 1 }} onPressEnter={handleSendComment} />
